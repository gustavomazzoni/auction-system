...
-- ----------------------------
CREATE TABLE IF NOT EXISTS `users` (`username` VARCHAR(50) NOT NULL, PRIMARY KEY (`username`));
CREATE TABLE IF NOT EXISTS `players` (`username` VARCHAR(50) NOT NULL, `coins` INT(8) NOT NULL, PRIMARY KEY (`username`));
CREATE TABLE IF NOT EXISTS `items` (`id` INT(8) NOT NULL AUTO_INCREMENT, `name` VARCHAR(50) NOT NULL, `avatar` VARCHAR(50), PRIMARY KEY (`id`));
CREATE TABLE IF NOT EXISTS `players_items` (`id` INT(8) NOT NULL AUTO_INCREMENT, `username` VARCHAR(50) NOT NULL, `id_item` INT(8) NOT NULL, `quantity` INT(8) NOT NULL, PRIMARY KEY (`id`), CONSTRAINT fk_player FOREIGN KEY (`username`) REFERENCES players(`username`), CONSTRAINT fk_item FOREIGN KEY (`id_item`) REFERENCES items(`id`));
